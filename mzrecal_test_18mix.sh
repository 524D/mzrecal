...
# Re-recalibration does not really help
# echo -n "Computing re-recalibration "
# $T "${TARGET_DIR}/mzrecal" -mzTry 12 -mzAccept=4 -scoreFilter="MS:1002257(0.0:0.05)" "${DATA_DIR}/${FN_BASE}-recal.mzML"
# echo -n "Creating recalibrated output "
# $T "${TARGET_DIR}/mzrecal" -recal "${DATA_DIR}/${FN_BASE}-recal.mzML"
#
# echo -n "Running msconvert (generating indexed mzml) "
# $T "${TOOLS_DIR}/msconvert" "${DATA_DIR}/${FN_BASE}-recal-recal.mzML" --outfile "${FN_BASE}-recal-recal.indexed.mzML" -o "${DATA_DIR}"  >/dev/null
# echo -n "Running comet on re-recalibrated output "
# $T "${TOOLS_DIR}/comet.2018012.linux.exe" -D${DATA_DIR}/${FASTA} "-P${TOOLS_DIR}/comet.params" "${DATA_DIR}/${FN_BASE}-recal-recal.indexed.mzML"  >/dev/null
#
# echo "Converting to .pep.xml "
# "${TOOLS_DIR}/idconvert" -o "${DATA_DIR}" "${DATA_DIR}/${FN_BASE}-recal-recal.indexed.pep.xml" >/dev/null 2>/dev/null
# # Note: idconvert does not handle files with multiple extensions correctly,
# # hence the .indexed extension is missing
# echo -n "Number of identifications with expectation value<0.01: "
# grep 'Comet:expectation value" value=".*E-.[3-9]' -P "${DATA_DIR}/${FN_BASE}-recal-recal.mzid" | wc -l
#
